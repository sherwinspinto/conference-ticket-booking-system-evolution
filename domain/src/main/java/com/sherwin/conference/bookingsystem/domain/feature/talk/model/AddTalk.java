...
public record AddTalk(Speaker speaker, TalkName talkName, TalkTime talkTime, SeatCount seatCount)
  ...
  public static List<Optional<FieldError>> validate(
    ...
    var op = TalkMetaData.Operation.ADD;
    List<Optional<FieldError>> validations = new ArrayList<>();
    validations.add(
        CommonValidations.validateEmptyString(
            TalkMetaData.TalkFields.NAME.at(op).toFieldName(), FieldValue.of(talkName)));
    validations.add(
        CommonValidations.validateOutOfRange(
            TalkMetaData.TalkFields.SEAT_COUNT.at(op).toFieldName(),
    ...
    validations.add(
        CommonValidations.validateNullObject(
            TalkMetaData.TalkFields.TALK_TIME.at(op).toFieldName(), FieldValue.of(talkTime)));
    if (talkTime != null)
      validations.addAll(TalkTime.validate(talkTime.startTime().value(), talkTime.endTime().value(), op));

    validations.add(
        CommonValidations.validateNullObject(
            TalkMetaData.TalkFields.SPEAKER.at(op).toFieldName(), FieldValue.of(speaker)));
    if (speaker != null)
