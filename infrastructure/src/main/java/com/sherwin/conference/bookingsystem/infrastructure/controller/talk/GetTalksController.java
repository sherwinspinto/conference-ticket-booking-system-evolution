...
public class GetTalksController {
  ...
  public ResponseEntity<List<TalkDto>> getAllTalks(Pageable pageable) {
    return ResponseEntity.ok(getTalksPort.getAllTalks(toDomain(pageable)).stream().map(TalkDto::fromDomain).toList());
  }

  private PageRequest toDomain(Pageable pageable) {
