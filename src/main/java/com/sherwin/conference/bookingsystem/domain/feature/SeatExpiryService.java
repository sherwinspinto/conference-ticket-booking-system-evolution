...
public class SeatExpiryService {
  private static final Logger LOGGER = LoggerFactory.getLogger(SeatExpiryService.class);
  private final ExpireOldReservationsAction expireOldReservationsAction;
  private final ScheduledExecutorService taskScheduler;
  private final ExecutorService virtualExecutor;

  public SeatExpiryService(
  ...
  public boolean expireSeat(SeatReserved seatReserved, long delay, TimeUnit timeUnit) {
    taskScheduler.schedule(
        ...
        delay,
        timeUnit);
    LOGGER.info("Scheduled Expiry Job for ticket: {} ----- with Delay: {} ----- TimeUnit: {} ", seatReserved.ticketId(), delay, timeUnit.name());
    return Boolean.TRUE;
  }

  void updateStatusToExpiredInThread(Long ticketId) {
    virtualExecutor.submit(() -> updateStatusToExpired(ticketId));
  }

  void updateStatusToExpired(Long ticketId) {
    expireOldReservationsAction.updateTicketStatusToExpired(ticketId);
    LOGGER.info("Updated status to EXPIRED for ticket: {}", ticketId);
  }
