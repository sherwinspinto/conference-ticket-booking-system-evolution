...
public interface TicketRepository extends JpaRepository<TicketEntity, Long> {
  @Query(
      value = "SELECT * FROM tickets t WHERE t.talk_id = :talkId AND t.status = :status",
      nativeQuery = true)
  List<TicketEntity> findByTalkIdAndStatus(
      @Param("talkId") Long talkId, @Param("status") String status);

  @Modifying
  @Query(
      value = "UPDATE Tickets t SET t.status = :status WHERE t.id = :ticketId",
      nativeQuery = true)
